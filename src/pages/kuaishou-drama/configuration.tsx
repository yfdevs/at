...
export function KuaishouDramaConfigurationPage() {
  const [config, setConfig] = useState<KuaishouDramaConfig>(emptyConfig)
  const [savedConfig, setSavedConfig] = useState<KuaishouDramaConfig>(emptyConfig)
  ...
  const applyResult = (result: KuaishouDramaConfigResult) => {
  ...
  const updateConfig = (key: keyof KuaishouDramaConfig, value: string) => {
  ...
  const saveConfig = async () => {
    setLoading(true)
    try {
      const result = await kuaishouDramaService.saveConfig(config)
      applyResult(result)
      ...
      <div className="sticky top-0 z-10 border-b bg-background/95 px-6 py-3 backdrop-blur">
        <div className="mx-auto flex w-full max-w-[860px] flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 flex-wrap items-center gap-2">
            <h1 className="mr-1 text-lg font-semibold tracking-normal">快手短剧配置</h1>
            <span
              className={
                hasChanges
      ...
      <div className="mx-auto flex w-full max-w-[760px] flex-1 flex-col gap-7 p-6">
        <section className="scroll-mt-28 space-y-3">
          <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground sm:text-sm">登录态和临时文件由快手短剧平台独立管理。</p>
          </div>
            ...
            <CardContent className="py-0">
              <FieldGroup className="gap-0">
                <Field className="gap-2.5 py-3 md:grid md:grid-cols-[minmax(220px,1fr)_280px] md:items-start">
                  <FieldContent>
                    <FieldLabel htmlFor="kuaishou-run-data-dir">运行数据目录</FieldLabel>
                    <FieldDescription>浏览器登录态位于该目录的 auth 子目录。</FieldDescription>
                  </FieldContent>
                  <InputGroup>
                    <InputGroupInput
                      id="kuaishou-run-data-dir"
                ...
                <Field className="gap-2.5 py-3 md:grid md:grid-cols-[minmax(220px,1fr)_280px] md:items-start">
                  <FieldContent>
                    <FieldLabel htmlFor="kuaishou-operation-delay">操作延迟</FieldLabel>
                    <FieldDescription>每一步 Playwright 操作之间的延迟。</FieldDescription>
                  </FieldContent>
                  <InputGroup>
                    <InputGroupInput
                      id="kuaishou-operation-delay"
                      min={0}
                      step="0.01"
                      type="number"
                ...
                <Field className="gap-2.5 py-3 md:grid md:grid-cols-[minmax(220px,1fr)_280px] md:items-center">
                  <FieldContent>
                    <FieldLabel htmlFor="kuaishou-headless">浏览器窗口</FieldLabel>
