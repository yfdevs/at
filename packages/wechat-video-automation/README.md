...
剧集视频不从任务数据的 `episodes` 字段读取。设置 `localEpisodeVideoRoot` 后，程序使用领取接口返回的 `originalTitle`，扫描 `根目录/originalTitle` 下的 `.mp4` 文件。源文件名支持 `originalTitle第N集.mp4`、`originalTitle-第N集.mp4` 和 `originalTitle - 第N集.mp4`。上传前会在运行数据目录创建硬链接，并把上传文件名设为 `payloadJson.name-第N集.mp4`；源视频和 `runDataDir` 需在同一磁盘分区。集数必须从 1 连续到 `playlet.episodeCount`，否则当前任务直接报错退出。
