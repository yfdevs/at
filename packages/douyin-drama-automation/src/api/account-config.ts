...
export type DouyinDramaAccount = {
  id: number;
  accountId: string;
  accountName: string;
...
export async function fetchDouyinDramaAccounts(
  apiBaseUrl: string,
  fetcher: typeof fetch = fetch,
): Promise<DouyinDramaAccount[]> {
  ...
  for (let page = 1; ; page += 1) {
    const response = await fetcher(`${baseUrl}/dramaAiRpa/douyin/accountConfig/page`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      throw new Error(`DOUYIN_DRAMA_ACCOUNT_CONFIG_REQUEST_FAILED: status=${response.status}`);
    }
    const payload = pageResponseSchema.parse(await response.json());
    if (payload.code !== 0) {
      throw new Error(
        `DOUYIN_DRAMA_ACCOUNT_CONFIG_REQUEST_FAILED: code=${payload.code} ` +
          `message=${payload.msg || "-"}`,
      );
    }
    if (!payload.data) throw new Error("DOUYIN_DRAMA_ACCOUNT_CONFIG_RESPONSE_DATA_REQUIRED");
  ...
  const unique = new Map<string, DouyinDramaAccount>();
