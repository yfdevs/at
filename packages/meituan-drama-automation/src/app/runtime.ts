...
export async function startMeituanCreationRuntime(
  ...
  log(options, "[meituan-drama] starting browser");
  ...
  void runPublishTask(context, page, options, taskConfig).catch((error) => {
    log(
      options,
      `[meituan-drama] task failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  });
  ...
  return {
    ...
    async stop() {
      ...
      running = false;
      await context?.close();
      log(options, "[meituan-drama] runtime skeleton stopped");
    },
  };
}
